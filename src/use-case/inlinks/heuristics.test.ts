...
test("normalizeForTokens: lowercases, strips punctuation, normalizes whitespace", () => {
  ...
  expect(normalizeForTokens("Inglês—básico\t\t(2026)")).toBe("inglês básico 2026");
...
test("isEarlyStageGettingStarted: detects early-stage / onboarding signals (pt-BR)", () => {
  ...
  expect(isEarlyStageGettingStarted("Guia completo de gramática avançada")).toBe(
    false,
  );
});

test("isLaterStageOrPrereqHeavy: detects later-stage / prerequisite-heavy topics", () => {
  expect(
  ...
  expect(isLaterStageOrPrereqHeavy("Como ser fluente em inglês")).toBe(true);
  expect(isLaterStageOrPrereqHeavy("Dicas para iniciantes em inglês")).toBe(false);
});
...
test("extractDistinctiveTokens: returns a small list and avoids overly generic cluster tokens", () => {
  ...
  expect(tokens.some((t) => ["intensivo", "imersão", "imersao", "cronograma"].includes(t))).toBe(
    true,
  );
});

test("anchorContainsDistinctiveToken: requires at least one distinctive token from destination", () => {
  const candidate =
    "Curso intensivo de inglês: cronograma intensivo, imersão e exercícios diários para acelerar resultados.";
  expect(anchorContainsDistinctiveToken("curso intensivo de inglês", candidate)).toBe(
    true,
  );

  // Generic anchor that doesn't include distinctive terms should fail
  expect(anchorContainsDistinctiveToken("curso de inglês", candidate)).toBe(false);
});
...
test("anchorTokenOverlap: counts overlap between anchor and candidate tokens", () => {
  ...
  const overlap = anchorTokenOverlap("aplicativos para aprender inglês", candidate);
