...
import { Send, Globe, Loader2, Sparkles, User, Bot, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'Desculpe, encontrei um erro ao processar sua solicitação. Por favor, verifique a conexão com o backend.',
        timestamp: Date.now(),
      };
    ...
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 bg-white border-r border-gray-100 p-6 flex flex-col gap-6 overflow-y-auto hidden md:flex">
          <div>
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Configuração</h2>
            
            <form id="config-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="space-y-1">
                <label htmlFor="url" className="text-sm font-medium text-gray-700">URL do Conteúdo <span className="text-red-500">*</span></label>
              ...
              <div className="space-y-1">
                <label htmlFor="intent" className="text-sm font-medium text-gray-700">Intenção (Opcional)</label>
                <select
                  id="intent"
                  value={intent}
              ...
              <div className="space-y-1">
                <label htmlFor="query" className="text-sm font-medium text-gray-700">Consulta Específica (Opcional)</label>
                <input
                  id="query"
                  type="text"
              ...
              <div className="space-y-1">
                <label htmlFor="tone" className="text-sm font-medium text-gray-700">Tom (Opcional)</label>
                <input
                  id="tone"
                  type="text"
          ...
          <div className="mt-auto">
             <div className="p-4 bg-orange-50 rounded-lg border border-orange-100">
                ...
                <p className="text-xs text-orange-700 leading-relaxed">
                  Forneça uma URL para começar. Se não selecionar uma intenção, a IA perguntará como você deseja usar o conteúdo.
                </p>
             </div>
          </div>
        </aside>
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 p-8">
                ...
                <p className="max-w-md mx-auto">
                  Insira uma URL na configuração lateral para gerar conteúdo para redes sociais, resumos e roteiros com IA.
                </p>
              </div>
            )}
                ...
                <div className={clsx(
                  ...
                  {msg.sources && msg.sources.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <p className="text-xs font-semibold text-gray-500 mb-2">Fontes:</p>
                      <ul className="space-y-1">
                        {msg.sources.map((source, idx) => (
                           <li key={idx}>
                             <a href={source} target='_blank' rel='noreferrer' className="text-xs text-primary hover:underline truncate block max-w-xs">{source}</a>
                           </li>
                        ))}
                      </ul>
                    </div>
